...
export const getFragances = async (): Promise<Fragances> => {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from<"fragrances", Fragance>("fragrances")
    .select(`*`);
...
export const getFraganceCategories = async (): Promise<FraganceCategories> => {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from<"fragrance_categories", FraganceCategory>("fragrance_categories")
    .select(`*`);
