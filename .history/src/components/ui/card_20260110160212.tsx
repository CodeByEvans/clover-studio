...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "group relative overflow-hidden rounded-xl border border-border bg-card text-card-foreground shadow-lg transition-all duration-300 hover:shadow-2xl",
        className
      )}
      {...props}
    />
...
function CardImage({
  ...
  return (
    <div
      ...
      className={cn("relative h-72 w-full overflow-hidden bg-muted", className)}
      {...props}
    >
      {src && (
        <img
...
function CardPrice({
  ...
  return (
    ...
    >
      <span className="text-2xl font-bold text-foreground" {...props} />
      {originalPrice && (
...
function CardAction({
  ...
  const variants = {
    default:
      "flex flex-1 items-center justify-center gap-2 rounded-lg bg-primary px-4 py-3 text-primary-foreground hover:bg-primary/90",
    icon: "flex h-12 w-12 items-center justify-center rounded-lg border border-border bg-background text-foreground hover:bg-accent hover:text-accent-foreground",
  };
