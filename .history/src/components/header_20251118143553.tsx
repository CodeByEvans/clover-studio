          ...
          <div className="flex items-center justify-between h-20">
            ...
            <nav className="hidden lg:flex items-center space-x-10">
              {collections.map((collection) => (
                <button
                  key={collection.id}
                  className="text-gray-700 hover:text-[#8B1E3F] font-medium transition-colors duration-200 hover:underline hover:underline-offset-4 cursor-pointer text-xs md:text-xs"
                >
                  {collection.title}
                </button>
              ))}
